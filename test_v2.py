...
def test():
    ...
    model.load_state_dict(torch.load(os.path.join(args.checkpoint_path, pth_name, "last_epoch_95.pth")))
    ...
    logging.info("Mean AUC: " + str(np.mean(patient_roc_auc)))
    logging.info("Overall ACC: %f" % ACC)
    logging.info("MCC: %f" % MCC)
    logging.info("Specificity (Spe): %f" % Spe)
    logging.info("Sensitivity (Sen): %f" % Sen)
