...
class TSNDataSet(data.Dataset):
    def __init__(self, root_path, list_file,
                 ...
                 remove_missing=False, dense_sample=False, twice_sample=False, data_mode='rgb'):
    ...
    def _load_image(self, directory, idx):
        if self.modality == 'RGB' or self.modality == 'RGBDiff':
            try:
                dir_path = Path(self.root_path, directory)
                # print(dir_path)
                pID_times = [ temp.stem for temp in dir_path.glob("*") ]
                pID_times.sort()
                temp = [ int(t) for t in pID_times[:-2] ]
                temp.sort()
                times = [ str(t) for t in temp ]
                times.extend( pID_times[-2:] )
                # print(times)
                tar_dir = Path( dir_path, times[idx-1] )
                # print(tar_dir)
                jpg_path = [ t for t in tar_dir.glob("*jpg")][0]
                # print(jpg_path)
                img = Image.open( str(jpg_path) ).convert("RGB")
                w,h = img.size
                pivot = int(w/2)
                json_path = Path( jpg_path.parent, jpg_path.stem+".json" )
                # print(json_path)
                with open( str(json_path), "r" ) as f:
                    data = json.load(f)
                min_x,min_y = data['shapes'][0]['points'][0]
                max_x,max_y = data['shapes'][0]['points'][1]
                if self.data_mode=='gray':
                    if min_x<pivot:
                        min_x = min_x + pivot
                        max_x  = max_x + pivot
                    else:
    ...
    def _sample_indices(self, record):
        ...
        if self.dense_sample:  # i3d dense sample
            ...
            # print(average_duration,55555555555555555555555555555555)
        ...
        for seg_ind in indices:
            ...
            for i in range(self.new_length):
                ...
                if p < record.num_frames:
                    p += 1
        # print(len(images),8888888888888888888888888888)

        process_data = self.transform(images)
        # print(process_data.shape,7777777777777777777777777777)
        # process_data = images
